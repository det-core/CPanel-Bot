...
global.resize = async (image, size1 = 100, size2 = 100) => {
	return new Promise(async(resolve, reject) => {
		try {
			const read = await Jimp.read(image);
			const result = await read.resize(size1, size2).getBufferAsync(Jimp.MIME_JPEG)
			resolve(result)
		} catch (e) {
			reject(e)
		}
...
global.fetchSubDomains = async function (zone, token) {
    try {
        ...
        console.error("Failed to fetch subdomains:", err.response?.data || err.message);
        return [];
    }
};
